...
![](genre groups.png){fig-align="center" width="82%"}
...
plot_qda_boundary <- function(qda_result, training_data, target_column_name,
  ...
  # Get fitted QDA model
  model <- qda_result$model
  
  # Create grid across two selected features
  grid <- expand.grid(
  ...
  # Fill all other predictor variables with their mean values
  ...
  # Make sure columns are in the same order as training predictors
  grid <- grid[, predictor_names]
  
  # Predict class for each grid point
  grid$predicted_class <- predict(model, newdata = grid)$class
  
  # Plot QDA decision regions with actual training points
  ggplot2::ggplot() +
    ggplot2::geom_tile(
      data = grid,
      ggplot2::aes(
    ...
    ggplot2::theme_minimal() +
    ggplot2::labs(
      title = "QDA Decision Boundary Plot",
...
```{r}
qda_plot <- plot_qda_boundary(
  qda_result = qda_results,
  training_data = training_and_test_data$training_data,
  target_column_name = "genre_group",
  x_feature = "energy",
  y_feature = "acousticness"
)
...
plot_fda_boundary <- function(fda_result, training_data, target_column_name,
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::theme_minimal() +
    ggplot2::labs(
      title = "FDA Decision Boundary Plot",
      subtitle = paste(x_feature, "vs", y_feature),
      x = x_feature,
      y = y_feature,
      fill = "Predicted Class",
...
```{r}
fda_plot <- plot_fda_boundary(
  fda_result = fda_results,
  training_data = training_and_test_data$training_data,
  target_column_name = "genre_group",
  x_feature = "energy",
  y_feature = "acousticness"
